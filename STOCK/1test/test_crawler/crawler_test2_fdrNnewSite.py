...
class Main:
    def __init__(self):
        ...
        self.codeurl = "http://data.krx.co.kr/contents/MDC/MDI/mdiLoader/index.cmd?menuId=MDC0201010106"

        self.data = []#append로 ls붙여넣을거임. (붙이면 2차원 배열 됨)
        self.ls_code=[]#append로 ls붙여넣을거임.(붙이면 2차원 배열 됨)


        html = requests.get(self.cudeurl)
        soup = BeautifulSoup(html, content, 'html.parse')
        html1 = soup.find('div',class_='CI-GRID-BODY-INNER')
        html2 = html1.find('table',class_='CI-GRID-BODY-TABLE')
        html3 = html2.find('tbody',class_'CI-GRID-BODY-TABLE-TBODY')
        html4 = html3.find_all('tr',class_='CI-GRID-ODD')
        k
        
        
        html = requests.get(se)
